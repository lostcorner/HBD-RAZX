...
当前版本不保存本地 MP3。回忆题先让用户凭记忆选择月份和课间，提交答案后再显示 Apple Music、网易云音乐、QQ 音乐和 Spotify 的外链。

这些链接目前是按歌名和歌手生成的搜索入口，不保证每个平台都能直接播放同一版本的歌曲；实际播放权限、地区限制和登录要求由对应平台决定。铃声档案页也会显示这些外链。

这样可以避免把受版权保护的音频文件提交到仓库，也不需要为音频配置存储服务。后续如果要接入真正的嵌入播放器，需要为每首歌维护平台对应的歌曲 ID，并逐个平台处理嵌入限制。
